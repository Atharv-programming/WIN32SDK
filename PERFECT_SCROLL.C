...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    static TCHAR szWindowTitle[] = TEXT("Perfect Scrollbar"); 
    ...
    if(hwnd == NULL)
    {
        MessageBox(
                NULL, 
                TEXT("Failed to create an Window"), 
                TEXT("CreateWindowEx"), 
                MB_ICONERROR | MB_TOPMOST
        ); 
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    static TCHAR szBufferMessage[] = TEXT("row number = %d, column number = %d"); 
    ...
    switch(uMsg)
    {
        ...
        case WM_PAINT: 
            ...
            iPaintEndVertical = min( NR_ROWS-1, iVScrollPosition + ps.rcPaint.bottom / cyChar);

            for(iRowCounter = iPaintBeginVertical; iRowCounter <= iPaintEndVertical; ++iRowCounter)
            {
                for(iColumnCounter = 0; iColumnCounter < NR_COLUMNS; ++iColumnCounter)
                {
                    iLength = wsprintf(szOutputMessage, szBufferMessage, iRowCounter, iColumnCounter); 
                    TextOut(hdc, 
                            (NR_MAX_CHARS * iColumnCounter - iHScrollPosition) * cxChar, 
